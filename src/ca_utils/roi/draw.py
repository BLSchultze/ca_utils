...
matplotlib.use("TKAgg", force=True)  # changing backend of matplotlib for pyplot to work
...
def plot_rois(background, layer, rois=None):
    nb_rois = len(rois)
    cols = [(1, 1, 1, 0)]  # translucent background
    cols.extend("rbgycmrbgycmrbgycmrbgycmrbgycm"[:nb_rois])
    cmap = matplotlib.colors.ListedColormap(cols)

    masks = np.zeros(rois[0].shape[1:])
    masks.shape
    for cnt, roi in enumerate(rois):
        masks[roi[layer, ...]] = cnt + 1

    plt.imshow(background[layer], cmap="Greys")
    plt.imshow(masks, cmap=cmap, alpha=0.2)
    plt.colorbar()
...
def draw(mean_img):
    ...
    Args:
        mean_img (_type_): image to draw ROI on (dims V,X,Y,C)
    ...
    while True:
        key_press = input(
            f"press number (0..{nb_layers-1}) to select layer, any key to draw another ROI in layer {layer}, or q to quit and save your ROIs: "
        )
        if key_press == "q":  # stop drawing ROIs upon 'q' press
            ...
            this_roi = np.zeros_like(mean_img, dtype=bool)  # init empty ROI stack
...
def extract_traces(stack, masks):
    """
    Args:
        stack - stacks (with dims T,V,X,Y,C)
        masks - list of M masks (each mask with dims V,X,Y)
    Returns:
        array of M traces T, C, M
    """
    masks = np.stack(masks, axis=-1)  # V X, Y,
    # traces = []
...
def main(root: str, datename: str, session: str):
    smooth_sigma = 1.5
    ...
    mean_img = np.mean(gCaMPdata, axis=0)
